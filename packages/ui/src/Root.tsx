...
function RootInner({
  ...
  const isStartupProviderLoginEntryOpen = false;
  ...
  useEffect(() => {
    // 账号登录已移除；清理 Renderer 展示态，不恢复磁盘 OAuth，也不注册回调或轮询。
    setUser(null);
    setIsRestoringOAuthSession(false);
    setOAuthError(null);
    setOAuthPollingActive(false);
  }, [setUser, setIsRestoringOAuthSession, setOAuthError, setOAuthPollingActive]);
